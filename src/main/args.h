 ...
 * position, and when a matching ArgumentDefinition is found, the ArgHandlerFn
 * is called. The current index is passes as a reference. If the parameter
 * expects to consume an additional argument, the ArgHandlerFn should move the
 * pointer after the consumed arguments.
...
/**
 * Parse command-line arguments. This will check each argument in the argv list against
 * the provided "defs" array of ArgumentDefinition structs. If the parameter matches
 * one of the provided definitions, the definition's associated ArgHandlerFn will be
 * called. Free the argv in the returned ArgParseResult when done.
 * @param argc       total number of args in the argv array
 * @param argv       array of argument values
 * @param defs       definitions for allowed command-line arguments
 * @param def_count  total number of definitions in the defs array
